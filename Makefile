...
ifeq (,$(wildcard $(FULLPATH)/nginx))
	...
	$(eval CMD := TEST_NGINX_BINARY="$(FULLPATH)/nginx" TEST_MODULE_PATH="$(FULLPATH)/build/" TEST_FILE_PATH="$(FILEPATH)" ASAN_OPTIONS=suppressions=suppressions.txt TEST_NGINX_CATLOG=1 prove $(FIFTYONEDEGREES_FORMATTER) -v tests/examples :: $(DATAFILE))
...
ifeq (,$(wildcard $(FULLPATH)/nginx))
	...
	$(eval CMD := TEST_NGINX_BINARY="$(FULLPATH)/nginx" TEST_NGINX_GLOBALS="load_module $(MODULEPATH);" TEST_NGINX_GLOBALS_HTTP="51D_file_path $(FILEPATH);" ASAN_OPTIONS=suppressions=suppressions.txt TEST_NGINX_CATLOG=1 prove $(FIFTYONEDEGREES_FORMATTER) -v $(TESTS) :: $(DATAFILE))
ifdef FIFTYONEDEGREES_TEST_OUTPUT
	$(CMD) > $(FIFTYONEDEGREES_TEST_OUTPUT)
else
	$(CMD)
